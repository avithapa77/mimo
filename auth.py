...
from db import create_session, get_session, get_user
import logging
...
def refresh_access_token(refresh_token: str) -> str:
    """
    Call this when the access token expires (after 1 hour).
    Returns a fresh access token without requiring Google login again.
    Valid for up to 30 days from original login.
    """
    session = get_session(refresh_token)   # raises if expired or not found
